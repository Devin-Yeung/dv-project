...
COMMON_WORDS <- c("a", "in", "at", "be", "of", "the", "an", "to", "on", "he", "she", "and", "his", "with", "her", "for", "their", "when", "this", "from", "as", "is", "by", "after", "that", "who", "but", "into", "up", "they", "him", "out", "must", "are", "about", "it", "its")
...
extract_words <- function(sentences) {
  strsplit(sentences, "\\s+") %>%
    ...
    map(tolower) %>%
    flatten_chr() %>%
    discard(function(x) length(x) <= 0) %>% # remove empty string
